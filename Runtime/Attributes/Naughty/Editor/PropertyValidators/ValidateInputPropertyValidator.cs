...
namespace NeutronNetwork.Naughty.Attributes.Editor
{
                ...
                validationCallback.ReturnType == typeof(bool))
            {
                ...
                if (callbackParameters.Length == 0)
                {
                    if (!(bool)validationCallback.Invoke(target, null))
                    {
                        ...
                        {
                            NaughtyEditorGUI.HelpBox_Layout(
                                validateInputAttribute.Message, (MessageType)validateInputAttribute.MessageType, context: property.serializedObject.targetObject);
                        }
                    }
                }
