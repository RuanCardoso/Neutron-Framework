...
namespace NeutronNetwork
{
    public class NeutronWriter : BinaryWriter, IDisposable
    {
        ...
        public void Write(NSVector3 writable)
        {
            Write(writable.x);
            Write(writable.y);
            Write(writable.z);
    ...
    public class NeutronReader : BinaryReader, IDisposable
    {
        ...
        public NSVector3 ReadSerializableVector3()
        {
            float x = ReadSingle();
            float y = ReadSingle();
            float z = ReadSingle();
        ...
        {
            if (len < sizeof(int)) LogHelper.Error($"The first bytes must be at least 4 Bytes, increasing the size of the buffer can solve.: {len}");
